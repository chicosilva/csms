...
class BRCPFCNPJField(Field):
	...
	default_error_messages = {
		...
		'max_digits': _(u"Este campo requer no máximo 11 dígitos para CPF ou 14 dígitos para CNPJ."),
	}
...
class ColorPickerWidget(TextInput):
    class Media:
        ...
        js = ('%sjs/jPicker/jpicker-1.1.6.js' % settings.STATIC_URL,)
...
class ZMoneyWidget(TextInput):
    ...
    show_symbol = None
    symbol_stay = None
    class Media: js = ('%sjs/jquery.maskMoney.js' % settings.STATIC_URL,)
    ...
    def render(self, name, value, attrs=None):
        ...
        try: attrs.update({'class':attrs.get('class').update(' mmony')})
        except: attrs.update({'class':' mmony'})
...
class ZEmailListWidget(TextInput):
    def __init__(self, **kwargs):
        super(ZEmailListWidget, self).__init__(kwargs)

    def render(self, name, value, attrs=None):
        if value: value = eval(value)
        if type(value) is list:
            value = ','.join(value)
        return super(ZEmailListWidget, self).render(name, value, attrs)


class HtmlWidget(MultiWidget):
    _html = ''
