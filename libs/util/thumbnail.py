...
class SEOThumbnailBackend(ThumbnailBackend):
    ...
    def _get_thumbnail_filename(self, source, geometry_string, options):
        ...
        name = split_path[0].split('/')[-1]
        path_parts = list(set(split_path[0].split('/')) - {name})
        split_path[0] = name
        split_path.insert(-1, geometry_string)

        #attempt to slugify the filename to make it SEO-friendly
        split_name = split_path[-1].split('.')
        try: split_path[-1] = '%s.%s' % (slugify('.'.join(split_name[:-1])), split_name[-1])
        except: pass
        split_path = path_parts + split_path
